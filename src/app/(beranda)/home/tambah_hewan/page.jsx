...
import { useAddAnimalMutation } from "@/hooks/animal.hooks";
...
export default function TambahHewanPage() {
  ...
  const { addAnimalMutation } = useAddAnimalMutation({
    successAction: () => {
      nav.push("/home");
    },
  });
  ...
  useEffect(() => {
    return () => {
      if (previewAnimalUrl) URL.revokeObjectURL(previewAnimalUrl);
    };
  }, [previewAnimalUrl]);
  ...
  const handleSubmit = () => {
    ...
    if (!animalFile) {
      toast.error("Upload foto hewan terlebih dahulu");
      return;
    }
    ...
    payload.append("hewan", animalFile);
    if (qrisFile) payload.append("file", qrisFile);

    addAnimalMutation.mutate({ payload });
  };
    ...
    <div className="min-h-screen px-6 py-8">
      <div className="max-w-5xl mx-auto bg-white rounded-3xl border border-gray-200 shadow-sm p-8">
        <div className="flex items-center justify-between mb-6">
          <Text className="text-xl font-semibold">Tambah Hewan Kamu</Text>
        ...
        <div className="grid gap-6">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="outline" onClick={() => nav.push("/home")}>Batal</Button>
            <Button type="button" disabled={addAnimalMutation.isPending} onClick={handleSubmit}>
              {addAnimalMutation.isPending ? "Menyimpan..." : "Tambah"}
            </Button>
          </div>
        </div>
      </div>
